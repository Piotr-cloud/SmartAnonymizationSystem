...
import dlib
import numpy as np
from NonPythonFiles.WorkersFiles.ContentSwappers.WuHukaiFaceSwapper import WuHukai_model_path
...
PREDICTOR_PATH = str(WuHukai_model_path)
...
def select_face(im, r=10, choose=False):
    ...
    points[:,0] = np.clip(points[:, 0], 0, face.shape[1] - 2)
    points[:,1] = np.clip(points[:, 1], 0, face.shape[0] - 2)
